...
void
AdaptiveFir::DumpAvgTransient(int decim_factor)
{
  int samp_idx;
  ...
  for (samp_idx = 0; samp_idx < Transient_Len; samp_idx += decim_factor) {
    trans_file << samp_idx << ", "
               ...
               << Sample_Transient[samp_idx] << std::endl;
  }
  trans_file.close();
}
void
AdaptiveFir::DumpTrajectory(double min_dist)
{
  int samp_idx;
...
void
AdaptiveFir::DumpAvgTrajectory(double min_dist)
{
  int samp_idx;
  double x, y, old_x, old_y, dist;
...
void
AdaptiveFir::DumpTransient(int decim_factor)
{
  int samp_idx;
  ...
  for (samp_idx = 0; samp_idx < Transient_Len; samp_idx += decim_factor) {
    trans_file << samp_idx << ", " << Sample_Transient[samp_idx] << std::endl;
  }
  trans_file.close();
}
void
AdaptiveFir::GetTaps(double* taps)
{
  int i;
