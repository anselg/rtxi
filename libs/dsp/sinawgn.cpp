...
SinesInAwgn::SinesInAwgn(double samp_intvl, istream& uin, ostream& uout)
{
  Samp_Intvl = samp_intvl;
  ...
  Ampl = new double[Num_Sines];
  for (int sin_idx = 0; sin_idx < Num_Sines; sin_idx++) {
    uout << "Desired frequency of sinusoid #" << sin_idx << " ?" << std::endl;
    uin >> (Freq[sin_idx]);
    Freq[sin_idx] *= TWO_PI;
    uout << "Desired phase of sinusoid #" << sin_idx << " ?" << std::endl;
    uin >> (Phase[sin_idx]);
    Phase[sin_idx] *= TWO_PI;
    uout << "Desired amplitude of sinusoid #" << sin_idx << " ?" << std::endl;
    uin >> (Ampl[sin_idx]);
  }
...
void
SinesInAwgn::GetNextSegment(complex* time_seq, int num_samps)
{
  ...
  for (samp_idx = 0; samp_idx < num_samps; samp_idx++) {
    ...
    for (sin_idx = 0; sin_idx < Num_Sines; sin_idx++) {
      value += Ampl[sin_idx] * sin(Freq[sin_idx] * time + Phase[sin_idx]);
    }
