                               ...
                               FirFilterDesign** fir_filter)
{
  ...
  for (j = 0; j < Num_PB_Freqs; j++)
    Ext_Freq[j] = (j + 1) * grid_density;

  for (j = 0; j < Num_SB_Freqs; j++)
    Ext_Freq[j + Num_PB_Freqs] = Num_Grid_Pts_PB + 1 + j * grid_density;
...
#ifdef _DEBUG
    DebugFile << "extremal_freqs[ " << j << " ] = " << extremal_freqs[j]
              << std::endl;
#endif
  }
  ...
  *fir_filter =
    new FirFilterDesign(filter_length, FIR_SYM_EVEN_LEFT, filter_coeffs);
...
void
RemezAlgorithm::RemezFinish(double* filter_coeffs)
{
  int k;
  double freq, *aa;
...
#ifdef _DEBUG
    DebugFile << "Coeff[ " << k << " ] = " << filter_coeffs[k] << std::endl;
#endif
  }
}
...
double
RemezAlgorithm::GetFrequency(int grid_index)
{
  if (grid_index <= Num_Grid_Pts_PB) {
    ...
    return (SB_Edge_Freq + (grid_index - (Num_Grid_Pts_PB + 1)) * SB_Increment);
  }
}
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

void
RemezAlgorithm::RemezError(void)
{
  int j;
  double freq;
  double ampl_resp;
...
double
RemezAlgorithm::ComputeRemezAmplitudeResponse(int init_flag, double contin_freq)
{
  ...
  if (init_flag) {
    ...
    for (k = 0; k <= Num_Approx_Funcs - 1; k++) {
      sign = -sign;
      freq = GetFrequency(Ext_Freq[k]);
      Gamma_[k] = DesiredResponse(freq) - sign * delta / WeightFunction(freq);
    }
  } // end of if(init_flag)
  else {
...
void
RemezAlgorithm::RemezSearch(void)
{
  ...
  if (((Error[0] > 0.0) && (Error[0] > Error[1]) &&
       (fabs(Error[0]) >= Abs_Delta)) ||
      ((Error[0] < 0.0) && (Error[0] < Error[1]) &&
       (fabs(Error[0]) >= Abs_Delta))) {
    Ext_Freq[k] = 0;
    k++;
  }
  ...
  for (j = 1; j < Num_Grid_Pts_PB; j++) {
    if (((Error[j] >= Error[j - 1]) && (Error[j] > Error[j + 1]) &&
         (Error[j] > 0.0)) ||
        ((Error[j] <= Error[j - 1]) && (Error[j] < Error[j + 1]) &&
         (Error[j] < 0.0))) {
      Ext_Freq[k] = j;
      k++;
    }
  ...
  j = Max_Grid_Indx;
  if (((Error[j] > 0.0) && (Error[j] > Error[j - 1]) &&
       (fabs(Error[j]) >= Abs_Delta)) ||
      ((Error[j] < 0.0) && (Error[j] < Error[j - 1]) &&
       (fabs(Error[j]) >= Abs_Delta))) {
  ...
  if (k > Num_Approx_Funcs + 1) {
    ...
    for (i = 1; i <= extras; i++) {
      ...
      indexOfSmallest = 0;
      for (j = 1; j < k; j++) {
        if (fabs(Error[Ext_Freq[j]]) >= minVal)
          continue;
...
RemezAlgorithm::RemezStop2()
{
  ...
  for (j = 1; j <= Num_Approx_Funcs; j++) {
    if (fabs(Error[Ext_Freq[j]]) < minVal)
      minVal = fabs(Error[Ext_Freq[j]]);
    if (fabs(Error[Ext_Freq[j]]) > maxVal)
      maxVal = fabs(Error[Ext_Freq[j]]);
  }
...
double
RemezAlgorithm::WeightFunction(double freq)
{
  double result;

  result = 1.0;
  if (freq <= PB_Edge_Freq)
    result = 1.0 / Ripple_Ratio;
  return (result);
}
...
double
RemezAlgorithm::DesiredResponse(double freq)
{
  double result;
