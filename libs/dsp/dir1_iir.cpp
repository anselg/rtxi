...
#ifdef _DEBUG
    DebugFile << numer_coeff[n] << " quantized to " << Quan_Numer_Coeff[n]
              << std::endl;
#endif
    Input_Buffer[n] = 0;
  }
...
#ifdef _DEBUG
    DebugFile << denom_coeff[n] << " quantized to " << Quan_Denom_Coeff[n]
              << std::endl;
#endif
    Output_Buffer[n] = 0;
  }
...
long
DirectFormIir::ProcessSample(long input_val)
{
  return (input_val);
}
double
DirectFormIir::ProcessSample(double input_val)
{
  double output_val;
  long term, sum;
  int input_read_indx, output_read_indx, tap_indx;

  Input_Buffer[Input_Write_Indx] = long(Input_Quan_Factor * input_val);
  ...
  for (tap_indx = 0; tap_indx < Num_Numer_Coeff; tap_indx++) {
    term = Quan_Numer_Coeff[tap_indx] * Input_Buffer[input_read_indx];
    sum += term;
    input_read_indx--;
    if (input_read_indx < 0)
      input_read_indx = Num_Numer_Coeff - 1;
  }

  output_read_indx = Output_Write_Indx;
  for (tap_indx = 1; tap_indx <= Num_Denom_Coeff; tap_indx++) {
    term = Quan_Denom_Coeff[tap_indx] * Output_Buffer[output_read_indx];
    sum += term;
    output_read_indx--;
...
int
DirectFormIir::GetNumNumerCoeff(void)
{
  return (Num_Numer_Coeff);
}

int
DirectFormIir::GetNumTaps(void)
{
  return (Num_Numer_Coeff);
}

int
DirectFormIir::GetNumDenomCoeff(void)
{
