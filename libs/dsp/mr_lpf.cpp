...
//----------------------------------------------------------
double
MultirateLowpass::ProcessSample(double input_val)
{
  double output_val;
  //-----------------------------------------------------
  //  Decimator section
  ...
  //
  output_val = Int_Rate * (Int_Filt[Int_Rho])->ProcessSample(Int_In);
  Int_Rho++;
  if (Int_Rho >= Int_Rate) {
    Int_Rho = 0;
  }
...
//----------------------------------------------------------
long
MultirateLowpass::ProcessSample(long input_val)
{
  long output_val;
  //-----------------------------------------------------
  //  Decimator section
  ...
  //  Interpolator section
  //
  output_val = Int_Rate * (Int_Filt[Int_Rho])->ProcessSample(Quan_Int_In);
  Int_Rho++;
  if (Int_Rho >= Int_Rate) {
    Int_Rho = 0;
  }
...
//
int
MultirateLowpass::GetNumTaps(void)
{
  return (Num_Taps);
}
