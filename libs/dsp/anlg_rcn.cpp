...
void
AnalogReconst::AddSample(double new_samp)
{
  int offset;
  double value;
  ...
  for (offset = 1; offset <= Max_Offset; offset++) {
    value = new_samp * sinc(double(offset * PI) / double(Analog_Interp_Rate));
    if (Curr_Samp >= offset)
...
void
AnalogReconst::CopyResult(double* output_array)
{
  int i;
  double* out_ptr;
...
AnalogReconst::FindPeak(void)
{
  ...
  peak_val = 0.0;
  for (i = Max_Offset; i < (Num_Analog_Samps - Max_Offset); i++) {
    if (Analog_Signal[i] > peak_val)
      peak_val = Analog_Signal[i];
  }
  return (peak_val);
}
//----------------------------------------------------------
...
double
AnalogReconst::FindPeakMag(void)
{
  int i;
  double peak_val;
                               ...
                               double cycles_per_corr)
{
  ...
  for (samp_idx = Max_Offset; samp_idx < (Num_Analog_Samps - Max_Offset);
    ...
    sum += (Analog_Signal[samp_idx] * cos(lambda * samp_idx + phase_offset));
  }
  return (sum / auto_sum);
}
