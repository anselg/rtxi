                                     ...
                                     ostream& uout)
{
  ...
  if (default_file_ok) {
    ...
    uout << "enter complete name for output file (30 chars max)" << std::endl;
    uin >> file_name;
...
//--------------------------------------------------
void
IirFilterResponse::ComputeResponse(void)
{
...
#ifdef _DEBUG
  DebugFile << " in IirFilterResponse::ComputeResponse" << std::endl;
#endif
...
#ifdef _DEBUG
  DebugFile << "in ComputeResponse, Num_Numer_Coeffs = " << Num_Numer_Coeffs
            << std::endl;
  DebugFile << "in ComputeResponse, Num_Denom_Coeffs = " << Num_Denom_Coeffs
            << std::endl;
#endif
...
void
IirFilterResponse::NormalizeResponse(void)
{
  int n;
  double biggest;
...
double*
IirFilterResponse::GetMagResp(void)
{
  return (Mag_Resp);
}
...
void
IirFilterResponse::DumpMagResp(void)
{
  ...
  for (int n = 0; n < Num_Resp_Pts; n++) {
    ...
    UnwrapPhase(n, &phase);
    (*Response_File) << freq << ", " << Mag_Resp[n] << ", " << phase
                     << std::endl;
  }
  return;
}
