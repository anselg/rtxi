                        ...
                        int num_taps_must_be_odd)
{
  ...
  total_mult_rate = 0.0;
  for (stage = 0; stage < num_stages; stage++) {
    low_stage_samp_rate = high_stage_samp_rate / stage_factor[stage];
    trans_width = low_stage_samp_rate - stopband_edge_freq - passband_edge_freq;
    std::cout << "trans_width = " << trans_width << std::endl;
    num_taps[stage] = (int)ceil(d_sub_inf * high_stage_samp_rate / trans_width);
    ...
    std::cout << "num taps for stage " << stage << " = " << num_taps[stage]
              << std::endl;
