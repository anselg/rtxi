...
void
CpfskSource::GetNextSegment(double* time_seq, int num_samps)
{
  ...
  while (cumul_samp_idx < num_samps) {
    if (Saved_Samp_Idx != 0) {
      for (samp_idx = Saved_Samp_Idx; samp_idx < Samps_Per_Symb; samp_idx++) {
...
void
CpfskSource::GetNextSegment(complex* time_seq, int num_samps)
{
  int samp_idx, cumul_samp_idx;
  double phi, rand_val, rad_freq;
  double td_energy;
  ...
  if (Seg_Num == 0)
    std::cout << "td_energy = " << td_energy << std::endl;
  Seg_Num++;
