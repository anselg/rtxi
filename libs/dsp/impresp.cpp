  ...
  for (r = 1; r <= Num_Poles; r++) {
    ...
    for (n = 1; n <= Num_Poles; n++) {
      if (n == r)
        continue;
#ifdef _DEBUG
      DebugFile << "pole[" << n << "] = " << (Trans_Func->GetPole(n))
                << std::endl;
      DebugFile << "difference term = " << (s_sub_r - (Trans_Func->GetPole(n)))
                << std::endl;
#endif
      k_sub_r_denom *= (s_sub_r - (Trans_Func->GetPole(n)));
#ifdef _DEBUG
      DebugFile << "k_sub_r_denom = " << k_sub_r_denom << std::endl;
#endif
    }
    for (n = 1; n <= Num_Zeros; n++) {
#ifdef _DEBUG
      DebugFile << "zero[" << n << "] = " << (Trans_Func->GetZero(n))
                << std::endl;
      DebugFile << "difference term = " << (s_sub_r - (Trans_Func->GetZero(n)))
                << std::endl;
#endif
      k_sub_r_numer *= (s_sub_r - (Trans_Func->GetZero(n)));
#ifdef _DEBUG
      DebugFile << "k_sub_r_numer = " << k_sub_r_numer << std::endl;
#endif
    }
    K_Sub_R[r] = k_sub_r_numer / k_sub_r_denom;
#ifdef _DEBUG
    DebugFile << "K_Sub_R[" << r << "] = " << K_Sub_R[r] << std::endl;
#endif
  }
...
//=========================================================
void
ImpulseResponse::GenerateResponse(void)
{
  int resp_indx;
...
//=========================================================
double
ImpulseResponse::ComputeSample(double time)
{
  int r;
  ...
  for (r = 1; r <= (Num_Poles >> 1); r++) {
    cos_part =
      2 * real(K_Sub_R[r]) * exp(Sigma[r] * time) * cos(Omega[r] * time);
    sin_part =
      2 * imag(K_Sub_R[r]) * exp(Sigma[r] * time) * sin(Omega[r] * time);
  ...
  if (Order_Is_Odd == 1) {
    h_of_t += (real(K_Sub_R[R_Max]) * exp(Sigma[R_Max] * time));
  }
  h_of_t *= H_Sub_Zero;
