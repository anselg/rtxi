...
void
FirFilterDesign::Initialize(int num_taps)
{
  Num_Taps = num_taps;
  Imp_Resp_Coeff = new double[num_taps];
  Original_Coeff = new double[num_taps];
...
void
FirFilterDesign::ScaleCoefficients(double scale_factor)
{
  int n;
...
void
FirFilterDesign::CopyCoefficients(double* coeff)
{
  for (int n = 0; n < Num_Taps; n++) {
...
void
FirFilterDesign::ApplyWindow(GenericWindow* window)
{
  for (int n = 0; n < Num_Taps; n++) {
...
void
FirFilterDesign::ExtractPolyphaseSet(double* coeff, int decim_rate, int rho)
{
  double* local_coeff;
  local_coeff = coeff;
