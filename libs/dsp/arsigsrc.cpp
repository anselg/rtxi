  ...
  *ret_samp_intvl = samp_intvl;
  ...
  switch (spec_meth) {
    ...
    case 2: // pole locations
      ...
      dig_filt_dsgn = BilinearTransf(anlg_filt_func, samp_intvl);
#ifdef _DEBUG
      ...
      if (mod_flag > 0) {
        ...
        for (ii = 1; ii <= cp_order; ii++) {
          ...
          uout << "denom_coeff[" << ii << "] = " << d_val << std::endl;
  ...
  for (ii = 0; ii < ar_order; ii++) {
    if (cabs(cp_roots[ii]) < 1.0)
      continue;
    uout << "generator root " << ii << " (val=" << cp_roots[ii]
         << ")\n is outside the unit circle" << std::endl;
  ...
  filt_implem =
    new UnquantDirectFormIir(1, cp_order, single_numer_coeff, denom_coeff);
  double noise_pwr;
