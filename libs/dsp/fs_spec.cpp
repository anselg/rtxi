...
FreqSampFilterSpec::FreqSampFilterSpec(istream& uin, ostream& uout)
{
  int n;
  double resp_val;
...
#define _BANDSTOP_RESP_ 4
  ...
  uout << "type of response ?\n"
       ...
       << "  4 = bandstop" << std::endl;
  uin >> Band_Config;
  if ((Band_Config < 1) || (Band_Config > 4))
    exit(0);
        ...
        case _LOWPASS_RESP_:
          ...
          if (Num_Taps % 2) // odd number of taps
          {
            if (Cutoff_Is_Half_Amp) {
              N1 =
                (int)floor(Num_Taps * Lambda_Numer / Lambda_Denom / 2.0 - 0.5);
          ...
          // Lambda = PI * Lambda;
          uout << "last sample in passband is sample [" << N1 << "]"
               << std::endl;
          uout << "with no transition samples,\n"
               << " predicted cutoff is " << lambda_predic << " times PI\n"
               << " or " << (lambda_predic * PI) << std::endl;
  ...
  if (Num_Trans_Samps == 0) {
    ...
    if (!Optimize_Enabled) {
      for (n = N1 + 1; n < N2; n++) {
        uout << "magnitude response sample [" << n << "] ?" << std::endl;
        uin >> resp_val;
...
void
FreqSampFilterSpec::SetTrans(double trans_val)
{
  switch (Band_Config) {
    case 1: // lowpass
...
//=============================================================
void
FreqSampFilterSpec::SetTrans(double* origins, double* slopes, double rho)
{
...
//==============================================================
void
FreqSampFilterSpec::SetMagRespSamp(int samp_indx, double value)
{
...
logical
FreqSampFilterSpec::IsOptimizeEnabled(void)
{
  return (Optimize_Enabled);
}
double
FreqSampFilterSpec::GetMagRespSamp(int samp_indx)
{
  return (Des_Mag_Resp[samp_indx]);
}

int
FreqSampFilterSpec::GetBandConfig(void)
{
  return (Band_Config);
}

int
FreqSampFilterSpec::GetNumTaps(void)
{
  return (Num_Taps);
}

int
FreqSampFilterSpec::GetNumTransSamps(void)
{
  return (Num_Trans_Samps);
}

int
FreqSampFilterSpec::GetN1(void)
{
  return (N1);
}

int
FreqSampFilterSpec::GetN2(void)
{
  return (N2);
}

int
FreqSampFilterSpec::GetN3(void)
{
  return (N3);
}

int
FreqSampFilterSpec::GetN4(void)
{
  return (N4);
}

int
FreqSampFilterSpec::GetFirType(void)
{
  return (Fir_Type);
}

void
FreqSampFilterSpec::SetBandConfig(int band_config)
{
  Band_Config = band_config;
}

void
FreqSampFilterSpec::SetNumTaps(int num_taps)
{
  Num_Taps = num_taps;
}

void
FreqSampFilterSpec::SetN1(int n1)
{
  N1 = n1;
}

void
FreqSampFilterSpec::SetN2(int n2)
{
  N2 = n2;
}

void
FreqSampFilterSpec::SetN3(int n3)
{
  N3 = n3;
}

void
FreqSampFilterSpec::SetN4(int n4)
{
  N4 = n4;
}

void
FreqSampFilterSpec::SetFirType(int fir_type)
{
  Fir_Type = fir_type;
}

void
FreqSampFilterSpec::SetDbScaleEnabled(int db_scale_enabled)
{
