                             ...
                             ostream& uout)
{
  ...
  for (resp_indx = 1; resp_indx < Num_Resp_Pts; resp_indx++) {
    lambda = resp_indx * Max_Sweep_Freq * 2.0 * PI * sampling_interval /
             (double)Num_Resp_Pts;
    ...
    //     (resp_indx * Max_Sweep_Freq * sampling_interval));
    for (samp_indx = 0; samp_indx < num_holdoff_samps; samp_indx++) {
    ...
    if (Db_Scale_Enabled) {
      Mag_Resp[resp_indx] = 20.0 * log10(max_output_mag);
    } else {
...
#ifdef _DEBUG
    DebugFile << "before normaliz, biggest Mag_Resp was " << biggest
              << std::endl;
#endif
    for (n = 1; n < Num_Resp_Pts; n++) {
...
SweptResponse::DumpMagResp(void)
{
  ...
  for (int n = 1; n < Num_Resp_Pts; n++) {
    ...
    (*Response_File) << freq << ", " << Mag_Resp[n] << std::endl;
  }
