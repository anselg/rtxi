...
rowvec<T>& rowvec<T>::operator*(matrix<T>& m2)
{
  ...
  v_res->Is_Temp = 1;
#ifdef _VEC_DEBUG
  DebugFile << "rv::op*(m): new rowvec at " << (void*)v_res << std::endl;
#endif
...
matrix<T>& colvec<T>::operator*(rowvec<T>& v2)
{
  ...
  matrix<T>* m_res = new matrix<T>(v1_orig, v1_len, v2_orig, v2_len);
  m_res->Is_Temp = 1;
#ifdef _VEC_DEBUG
...
vector<T>::vector(int origin, int size)
{
  ...
  p->f = new T[size];
#ifdef _VEC_DEBUG
  DebugFile << "v::v(i,i): array " << size << " long alloc at " << (void*)(p->f)
            << std::endl;
#endif
...
{
  return p->f[(
    ((i >= (p->orig_indx)) && (i <= p->max_indx)) ? (i - (p->orig_indx)) : 0)];
}
//-----------------------------------------------------
