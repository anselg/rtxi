...
//--------------------------------------------------
void
FirFilterResponse::ComputeMagResp(void)
{
  ...
  for (resp_indx = 0; resp_indx < Num_Resp_Pts; resp_indx++) {
    ...
    for (tap_indx = 0; tap_indx < Num_Taps; tap_indx++) {
      work =
        work + (coeff[tap_indx] * complex<double>(cos(tap_indx * lambda),
                                                  -sin(tap_indx * lambda)));
    }
...
void
FirFilterResponse::NormalizeResponse(void)
{
  int n;
  double biggest;
...
double*
FirFilterResponse::GetMagResp(void)
{
  return (Mag_Resp);
}
