                                   ...
                                   ostream& uout)
{
  ...
  if (Max_Sweep_Freq > (0.5 / sampling_interval)) {
    uout << "maximum swept frequency will be limited\n"
         << "to folding frequency of " << (0.5 / sampling_interval)
         << std::endl;
  ...
  for (resp_indx = 1; resp_indx < Num_Resp_Pts; resp_indx++) {
    lambda = TWO_PI * resp_indx * Max_Sweep_Freq * sampling_interval /
             (double)Num_Resp_Pts;
    norm_freq = resp_indx * Max_Sweep_Freq / (double)Num_Resp_Pts;
    samps_per_corr =
      int(Num_Resp_Pts * cycles_per_corr /
    ...
    for (phase_indx = -max_worsening_phases;
      ...
      if (num_worsening_phases > max_worsening_phases)
        break;
    }
    //---------------------------------------
    // "unwrap" phase to keep it all negative

    old_phase_lag = phase_lag - max_worsening_phases * phase_delta;
    ...
    if (Db_Scale_Enabled) {
      Mag_Resp[resp_indx] = 20.0 * log10(max_correl);
      // Mag_Resp[resp_indx] =
      //     20.0 * log10(peak_mag);
    } else {
...
IirSweptResponse::DumpMagResp(void)
{
  ...
  for (int n = 1; n < Num_Resp_Pts; n++) {
    ...
    (*Response_File) << freq << ", " << Mag_Resp[n] << ", " << Phase_Resp[n]
                     << std::endl;
  }
