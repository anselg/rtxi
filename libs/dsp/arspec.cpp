                            ...
                            logical db_plot_enab)
{
  ...
  if (ref_spectrum != NULL)
    ref_offset = 10.0 * log10(ref_spectrum->GetPsdValue(0.0));
  for (i = 0; i < Num_Pts; i++) {
    freq = i * Freq_Delt;
    if (ref_spectrum != NULL)
      ref_value = ref_spectrum->GetPsdValue(freq / Samp_Intvl);
    if (db_plot_enab) {
      out_file << freq << ", " << ((10.0 * log10(Spec_Buf[i])) - vert_offset)
               << ", " << ((10.0 * log10(ref_value)) - ref_offset) << std::endl;
    } else {
      out_file << freq << ", " << (Spec_Buf[i]) << ", " << ref_value
               << std::endl;
    }
  }
  out_file.close();
...
ArSpectrum<T>::DumpSpectrum(char* out_file_nam, logical db_plot_enab)
{
  ...
  for (i = 0; i < Num_Pts; i++) {
    ...
    if (db_plot_enab) {
      ...
      out_file << freq << ", " << (Spec_Buf[i]) << std::endl;
    }
  }
  out_file.close();
}
