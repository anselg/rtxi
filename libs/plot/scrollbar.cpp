...
void
ScrollBar::setInverted(bool inverted)
{
...
void
ScrollBar::moveSlider(double min, double max)
{
  const int sliderTicks =
    qRound((max - min) / (d_maxBase - d_minBase) * d_baseTicks);
...
double
ScrollBar::minBaseValue() const
{
  return d_minBase;
}

double
ScrollBar::maxBaseValue() const
{
  return d_maxBase;
}

void
ScrollBar::sliderRange(int value, double& min, double& max) const
{
  if (isInverted())
...
double
ScrollBar::minSliderValue() const
{
  double min, dummy;
...
double
ScrollBar::maxSliderValue() const
{
  double max, dummy;
...
int
ScrollBar::mapToTick(double v) const
{
...
double
ScrollBar::mapFromTick(int tick) const
{
  return d_minBase + (d_maxBase - d_minBase) * tick / d_baseTicks;
}

void
ScrollBar::catchValueChanged(int value)
{
  double min, max;
