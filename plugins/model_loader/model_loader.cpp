...
{
  action = MainWindow::getInstance()->createModuleMenuItem(
    "Load Plugin", this, SLOT(load(void)));
...
ModelLoader::load(void)
{
  ...
  QString filename = QFileDialog::getOpenFileName(
    0, tr("Load plugin"), plugin_dir, tr("Plugins (*.so);;All (*.*)"));

  if (filename.isNull() || filename.isEmpty() ||
      filename.contains("model_loader") || filename.contains("analogy"))
    return;
  ...
  if (doesnotexist) {
    if (num_module == 10) {
      userprefs.setValue("/recentFileList/" + QString::number(oldestmodule),
                         filename);
      index = oldestmodule;
      oldestmodule++;
      if (oldestmodule == 10)
      ...
      userprefs.setValue("/recentFileList/" + QString::number(num_module++),
                         filename);
      index = num_module;
