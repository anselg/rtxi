...
QDebug
operator<<(QDebug str, const QEvent* ev)
{
...
static void
findDAQDevice(DAQ::Device* dev, void* arg)
{
...
namespace {
void
buildBlockPtrList(IO::Block* block, void* arg)
{
...
int
InsertChannelEvent::callback(void)
{
  if (recording)
    return -1;
...
int
RemoveChannelEvent::callback(void)
{
  if (recording)
    return -1;
...
int
StartRecordingEvent::callback(void)
{
  DataRecorder::data_token_t token;
  recording = true;
...
int
StopRecordingEvent::callback(void)
{
  DataRecorder::data_token_t token;
  recording = false;
...
void
DataRecorder::CustomEvent::setData(void* ptr)
{
  data = ptr;
}

void*
DataRecorder::CustomEvent::getData(void)
{
  return data;
}

void
DataRecorder::startRecording(void)
{
...
void
DataRecorder::stopRecording(void)
{
...
void
DataRecorder::openFile(const QString& filename)
{
...
void
DataRecorder::postAsyncData(const double* data, size_t size)
{
...
DataRecorder::Panel::Panel(QWidget* parent, size_t buffersize)
  ...
  , recording(false)
{
  setWhatsThis("<p><b>Data Recorder:</b><br>The Data Recorder writes data to "
               "an HDF5 file format "
               "All available signals for saving to file are automatically "
               "detected. Currently "
               ...
               "to select the signals that you want to save. Use the left and "
               "right arrow buttons to "
               "add these signals to the file. You may select a downsampling "
               "rate that is applied "
               "to the real-time period for execution (set in the System "
               "Control Panel). The real-time "
               "period and the data downsampling rate are both saved as "
               "metadata in the HDF5 file "
               "so that you can reconstruct your data correctly. The current "
               "recording status of "
  ...
  QObject::connect(
    blockList, SIGNAL(activated(int)), this, SLOT(buildChannelList(void)));
  ...
  QObject::connect(
    typeList, SIGNAL(activated(int)), this, SLOT(buildChannelList(void)));
  ...
  QObject::connect(
    rButton, SIGNAL(released(void)), this, SLOT(insertChannel(void)));
  rButton->setEnabled(false);
  lButton = new QPushButton("Remove");
  channelLayout->addWidget(lButton);
  QObject::connect(
    lButton, SIGNAL(released(void)), this, SLOT(removeChannel(void)));
  ...
  QObject::connect(
    fileChangeButton, SIGNAL(released(void)), this, SLOT(changeDataFile(void)));
  ...
  QObject::connect(stopRecordButton,
                   SIGNAL(released(void)),
                   this,
                   SLOT(stopRecordClicked(void)));
  buttonLayout->addWidget(stopRecordButton);
  stopRecordButton->setEnabled(false);
  closeButton = new QPushButton("Close");
  QObject::connect(
    closeButton, SIGNAL(released(void)), subWindow, SLOT(close()));
...
// Execute loop
void
DataRecorder::Panel::execute(void)
{
...
// Event handler
void
DataRecorder::Panel::receiveEvent(const Event::Object* event)
{
...
// RT Event Handler
void
DataRecorder::Panel::receiveEventRT(const Event::Object* event)
{
...
void
DataRecorder::Panel::buildChannelList(void)
{
  channelList->clear();
  if (!blockList->count())
    return;
...
void
DataRecorder::Panel::changeDataFile(void)
{
...
void
DataRecorder::Panel::insertChannel(void)
{
...
void
DataRecorder::Panel::removeChannel(void)
{
...
void
DataRecorder::Panel::addNewTag(void)
{
...
void
DataRecorder::Panel::startRecordClicked(void)
{
  if (fileNameEdit->text().isEmpty()) {
...
// Stop recording slot
void
DataRecorder::Panel::stopRecordClicked(void)
{
  StopRecordingEvent RTevent(recording, fifo);
  RT::System::getInstance()->postEvent(&RTevent);
}
...
void
DataRecorder::Panel::updateDownsampleRate(int r)
{
  downsample_rate = r;
}
...
void
DataRecorder::Panel::customEvent(QEvent* e)
{
...
void
DataRecorder::Panel::doDeferred(const Settings::Object::State& s)
{
...
void
DataRecorder::Panel::doLoad(const Settings::Object::State& s)
{
...
void
DataRecorder::Panel::doSave(Settings::Object::State& s) const
{
  if (isMaximized())
...
void*
DataRecorder::Panel::bounce(void* param)
{
...
void
DataRecorder::Panel::processData(void)
{
  enum
  {
    CLOSED,
      ...
      if (block && state == RECORD) {
        ...
        QString parameter_name =
          ...
          QString::fromStdString(
            block->getName(Workspace::PARAMETER, data.index));
...
int
DataRecorder::Panel::openFile(QString& filename)
{
#ifdef DEBUG
  ...
  if (QFile::exists(filename)) {
    ...
    file.id = H5Fcreate(
      filename.toLatin1().constData(), H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
    trialNum->setText("0");
  }
  if (file.id < 0) {
    ...
    ERROR_MSG("DataRecorder::Panel::processData : failed to open \"%s\" for "
              "writing with error : %s\n",
...
void
DataRecorder::Panel::closeFile(bool shutdown)
{
#ifdef DEBUG
  if (!pthread_equal(pthread_self(), thread)) {
  ...
  if (!dataTags.empty()) {
    ...
    for (std::vector<std::string>::iterator it = dataTags.begin();
      ...
      data = H5Dcreate(file.tdata,
                       ...
                       H5P_DEFAULT);
      status =
        H5Dwrite(data, tag_type, H5S_ALL, H5S_ALL, H5P_DEFAULT, it->c_str());
    }
    dataTags.clear();
...
int
DataRecorder::Panel::startRecording(long long timestamp)
{
#ifdef DEBUG
  if (!pthread_equal(pthread_self(), thread)) {
    ERROR_MSG(
  ...
  file.trial = H5Gcreate(file.id,
                         ...
                         H5P_DEFAULT);
  file.pdata =
    H5Gcreate(file.trial, "Parameters", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  ...
  for (RT::List<Channel>::iterator i = channels.begin(), end = channels.end();
    ...
    hid_t data = H5Dcreate(file.sdata,
                           ...
                           H5P_DEFAULT);
    H5Dwrite(
      data, string_type, H5S_ALL, H5S_ALL, H5P_DEFAULT, rec_chan_name.c_str());
    H5Dclose(data);
  }
...
void
DataRecorder::Panel::stopRecording(long long timestamp)
{
#ifdef DEBUG
  if (!pthread_equal(pthread_self(), thread)) {
    ERROR_MSG(
...
DataRecorder::Plugin::Plugin(void)
{
  ...
  MainWindow::getInstance()->createSystemMenuItem(
    "Data Recorder", this, SLOT(createDataRecorderPanel(void)));
}
...
DataRecorder::Panel*
DataRecorder::Plugin::createDataRecorderPanel(void)
{
  Panel* panel =
...
void
DataRecorder::Plugin::removeDataRecorderPanel(DataRecorder::Panel* panel)
{
  panelList.remove(panel);
}

void
DataRecorder::Plugin::doDeferred(const Settings::Object::State& s)
{
  size_t i = 0;
...
void
DataRecorder::Plugin::doLoad(const Settings::Object::State& s)
{
  for (size_t i = 0; i < static_cast<size_t>(s.loadInteger("Num Panels"));
       ++i) {
    Panel* panel =
      new Panel(MainWindow::getInstance()->centralWidget(), buffersize);
    panelList.push_back(panel);
...
void
DataRecorder::Plugin::doSave(Settings::Object::State& s) const
{
...
DataRecorder::Plugin*
DataRecorder::Plugin::getInstance(void)
{
  if (instance)
    return instance;
