...
void
RTXIWizard::Panel::initParameters(void)
{
...
// and Install" if not.
void
RTXIWizard::Panel::updateButton(void)
{
...
void
RTXIWizard::Panel::cloneModule(void)
{
  cloneButton->setEnabled(false);
  ...
  if ((QDir(modules[name].location.toString())).exists()) {
    ...
    // printGitError(git_remote_connect(remote, GIT_DIRECTION_FETCH, NULL,
    // NULL));
#else
 ...
 */
void
RTXIWizard::Panel::getReadme(void)
{
  availableListWidget->setDisabled(true);
  installedListWidget->setDisabled(true);
...
void
RTXIWizard::Panel::parseReadme(void)
{
...
void
RTXIWizard::Panel::parseRepos(void)
{
  ...
  for (int idx = 0; idx < jsonArr.size(); idx++) {
    ...
    if (std::find(exclude_list.begin(),
                  exclude_list.end(),
                  newObj.value("name").toString()) == exclude_list.end()) {
      module_t module;

      QString name = newObj.value("name").toString();
      module.readme_url = QUrl(
        readmeUrlPrefix + newObj.value("name").toString() + readmeUrlSuffix);
...
void
RTXIWizard::Panel::rebuildListWidgets(void)
{
  availableListWidget->clear();
  installedListWidget->clear();

  for (std::map<QString, module_t>::iterator i = modules.begin();
       i != modules.end();
       ++i) {
    if (i->second.installed)
      installedListWidget->addItem(i->first);
    else
      availableListWidget->addItem(i->first);
  }
 ...
 */
void
RTXIWizard::Panel::installFromString(std::string module_name)
{
...
int
RTXIWizard::Panel::printGitError(int error)
{
  if (error) {
  ...
  : panel(0)
{
  MainWindow::getInstance()->createSystemMenuItem(
    "Plugin Manager", this, SLOT(showRTXIWizardPanel(void)));
}
...
void
RTXIWizard::Plugin::showRTXIWizardPanel(void)
{
  if (!panel)
...
void
RTXIWizard::Plugin::removeRTXIWizardPanel(RTXIWizard::Panel* p)
{
  if (p == panel)
    panel = NULL;
}
...
RTXIWizard::Plugin*
RTXIWizard::Plugin::getInstance(void)
{
  if (instance)
    return instance;
