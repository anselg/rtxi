  ...
  : QWidget(parent)
{
  ...
  setWhatsThis("<p><b>Connector:</b><br>The Connector panel allows you to make "
               "connections between "
               "signals and slots in your workspace. Signals are generated by "
               "the DAQ card (associated "
               "with input channels) and by user modules. Available signals "
               "are listed in the \"Output "
               "Block\" drop-down box and available slots are listed in the "
               "\"Input Block\" drop-down box. "
               "The arrow button is a toggle button that turns connections on "
               "and off. Clicking the toggle "
               "button immediately makes a connection active or inactive in "
               "real-time. Current connections "
...
void
Connector::Panel::receiveEvent(const Event::Object* event)
{
  if (event->getName() == Event::IO_BLOCK_INSERT_EVENT) {
    IO::Block* block = reinterpret_cast<IO::Block*>(event->getParam("block"));

    inputBlock->addItem(QString::fromStdString(block->getName()) +
                        QString(" ") + QString::number(block->getID()));
    outputBlock->addItem(QString::fromStdString(block->getName()) +
                         QString(" ") + QString::number(block->getID()));
    ...
    size_t index;
    for (index = 0; index < connectionBox->count() &&
...
void
Connector::Panel::buildInputChannelList(void)
{
  inputChannel->clear();
  if (!inputBlock->count())
    return;
...
void
Connector::Panel::buildOutputChannelList(void)
{
  outputChannel->clear();
...
void
Connector::Panel::highlightConnectionBox(QListWidgetItem* item)
{
  ...
  for (index = 0; index < blocks.size() && blocks[index] != src; ++index)
    ;
  if (index >= blocks.size())
    ERROR_MSG("Connector::Panel::highlightConnectionBox : highlighted source "
              "does not exist.\n");
  ...
  for (index = 0; index < blocks.size() && blocks[index] != dest; ++index)
    ;
  if (index >= blocks.size())
    ERROR_MSG("Connector::Panel::highlightConnectionBox : highlighted "
...
void
Connector::Panel::toggleConnection(bool on)
{
...
void
Connector::Panel::updateConnectionButton(void)
{
...
void
Connector::Panel::buildConnectionList(IO::Block* src,
                                      size_t src_num,
                                      IO::Block* dest,
                                      size_t dest_num,
                                      void* arg)
...
void
Connector::Plugin::showConnectorPanel(void)
{
  if (!panel)
...
void
Connector::Plugin::removeConnectorPanel(Connector::Panel* p)
{
  if (p == panel)
    panel = NULL;
}
...
Connector::Plugin*
Connector::Plugin::getInstance(void)
{
  if (instance)
    return instance;
