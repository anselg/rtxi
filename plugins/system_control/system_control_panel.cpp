...
static void
buildDAQDeviceList(DAQ::Device* dev, void* arg)
{
  ...
  : QWidget(parent)
{
  ...
  setWhatsThis(
    ...
    "configure "
    "the channels on your DAQ card. RTXI automatically detects the number and "
    "types "
    ...
    "combination of hardware and software that you are using. To acquire data "
    "from "
    ...
    "real-time "
    "execution. You must click \"Apply\" to change this setting and propagate "
    "it "
...
SystemControlPanel::apply(void)
{
  ...
  if (dev) {
    ...
    double a_zerooffset =
      analogZeroOffsetEdit->text().toDouble() *
    ...
    dev->setAnalogReference(
      a_type, a_chan, analogReferenceList->currentIndex());
...
void
SystemControlPanel::updateDevice(void)
{
  DAQ::Device* dev;
  DAQ::type_t type;
  {
...
void
SystemControlPanel::updateFreq(void)
{
  int i = 0;
  double freq;
  double period;
...
void
SystemControlPanel::updatePeriod(void)
{
  int i = 0;
  double freq, period;
...
void
SystemControlPanel::display(void)
{
  ...
  if (!dev || !analogChannelList->count()) {
    ...
    DAQ::type_t type =
      static_cast<DAQ::type_t>(analogSubdeviceList->currentIndex());
    DAQ::index_t chan =
      static_cast<DAQ::index_t>(analogChannelList->currentIndex());
  ...
  if (!dev || !digitalChannelList->count()) {
    ...
    DAQ::type_t type =
      static_cast<DAQ::type_t>(digitalSubdeviceList->currentIndex() + DAQ::DIO);
    DAQ::index_t chan =
      static_cast<DAQ::index_t>(digitalChannelList->currentIndex());
...
void
SystemControlPanel::receiveEvent(const Event::Object* event)
{
