  ...
  , dsc(*d)
{
  ...
  for (int i = 0; i < dsc.nb_subd; i++) {
    ...
    if (err != 0) {
      ERROR_MSG("AnalogyDriver: a4l_get_subd_info failed, wrong subdevice "
                "index %i (err=%d)\n",
                i,
                err);
    }
...
bool
AnalogyDevice::analog_exists(type_t type, index_t count) const
{
...
size_t
AnalogyDevice::getChannelCount(type_t type) const
{
...
bool
AnalogyDevice::getChannelActive(type_t type, index_t channel) const
{
...
int
AnalogyDevice::setChannelActive(type_t type, index_t channel, bool state)
{
  if (channel >= getChannelCount(type))
    return -EINVAL;
...
size_t
AnalogyDevice::getAnalogRangeCount(type_t type, index_t channel) const
{
...
size_t
AnalogyDevice::getAnalogReferenceCount(type_t type, index_t channel) const
{
  if (!analog_exists(type, channel))
    return 0;
...
size_t
AnalogyDevice::getAnalogUnitsCount(type_t type, index_t channel) const
{
  if (!analog_exists(type, channel))
    return 0;
...
size_t
AnalogyDevice::getAnalogDownsample(type_t type, index_t channel) const
{
  if (!analog_exists(type, channel))
    return 0;
...
std::string
AnalogyDevice::getAnalogRangeString(type_t type,
                                    index_t channel,
                                    index_t index) const
{
  if (!analog_exists(type, channel) ||
      !((index >= 0) && (index < getAnalogRangeCount(type, channel))))
    return "";
...
std::string
AnalogyDevice::getAnalogReferenceString(type_t type,
                                        index_t channel,
                                        index_t index) const
{
  if (!analog_exists(type, channel) ||
      !((index >= 0) && (index < getAnalogReferenceCount(type, channel))))
    return "";
...
std::string
AnalogyDevice::getAnalogUnitsString(type_t type,
                                    index_t channel,
                                    index_t index) const
{
  if (!analog_exists(type, channel) ||
      !((index >= 0) && (index < getAnalogUnitsCount(type, channel))))
    return "";
...
index_t
AnalogyDevice::getAnalogRange(type_t type, index_t channel) const
{
  if (!analog_exists(type, channel))
    return 0 - 1;
...
index_t
AnalogyDevice::getAnalogReference(type_t type, index_t channel) const
{
  if (!analog_exists(type, channel))
    return 0 - 1;
...
index_t
AnalogyDevice::getAnalogUnits(type_t type, index_t channel) const
{
  if (!analog_exists(type, channel))
    return 0 - 1;
...
index_t
AnalogyDevice::getAnalogOffsetUnits(type_t type, index_t channel) const
{
  if (!analog_exists(type, channel))
    return 0 - 1;
...
double
AnalogyDevice::getAnalogGain(type_t type, index_t channel) const
{
  if (!analog_exists(type, channel))
    return 0;
...
double
AnalogyDevice::getAnalogZeroOffset(type_t type, index_t channel) const
{
  if (!analog_exists(type, channel))
    return 0;
...
int
AnalogyDevice::setAnalogRange(type_t type, index_t channel, index_t index)
{
  if (!analog_exists(type, channel) ||
      !((index >= 0) && (index < getAnalogRangeCount(type, channel))))
    return -EINVAL;
...
int
AnalogyDevice::setAnalogReference(type_t type, index_t channel, index_t index)
{
  if (!analog_exists(type, channel) ||
      !((index >= 0) && (index < getAnalogReferenceCount(type, channel))))
    return -EINVAL;
...
int
AnalogyDevice::setAnalogUnits(type_t type, index_t channel, index_t index)
{
  if (!analog_exists(type, channel) ||
      !((index >= 0) && (index < getAnalogUnitsCount(type, channel))))
    return -EINVAL;
...
int
AnalogyDevice::setAnalogOffsetUnits(type_t type, index_t channel, index_t index)
{
  if (!analog_exists(type, channel) ||
      !((index >= 0) && (index < getAnalogUnitsCount(type, channel))))
    return -EINVAL;
...
int
AnalogyDevice::setAnalogZeroOffset(type_t type, index_t channel, double offset)
{
  if (!analog_exists(type, channel))
    return -EINVAL;
...
int
AnalogyDevice::setAnalogGain(type_t type, index_t channel, double gain)
{
  if (!analog_exists(type, channel))
    return -EINVAL;
...
int
AnalogyDevice::setAnalogCalibrationValue(type_t type,
                                         index_t channel,
                                         double value)
{
  if (!analog_exists(type, channel))
...
double
AnalogyDevice::getAnalogCalibrationValue(type_t type, index_t channel) const
{
  if (!analog_exists(type, channel))
    return -EINVAL;
...
int
AnalogyDevice::setAnalogDownsample(type_t type,
                                   index_t channel,
...
int
AnalogyDevice::setAnalogCounter(type_t type, index_t channel)
{
  if (!analog_exists(type, channel))
    return -EINVAL;
...
direction_t
AnalogyDevice::getDigitalDirection(index_t channel) const
{
...
int
AnalogyDevice::setDigitalDirection(index_t channel, direction_t direction)
{
  if (channel >= subdevice[DIO].count)
    return -EINVAL;
...
void
AnalogyDevice::doLoad(const Settings::Object::State& s)
{
...
void
AnalogyDevice::doSave(Settings::Object::State& s) const
{
