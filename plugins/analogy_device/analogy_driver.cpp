...
DAQ::Device*
AnalogyDriver::createDevice(const std::list<std::string>& args)
{
  int err = 0;
  a4l_desc_t dsc;
  a4l_sbinfo_t* sbinfo;
  ...
  for (size_t i = count[0] + count[1]; i < count[0] + count[1] + count[2];
       ++i) {
    std::ostringstream name;
  ...
  AnalogyDevice* dev =
    new AnalogyDevice(&dsc, name, channel, count[0] + count[1] + 2 * count[2]);
...
void
AnalogyDriver::doLoad(const Settings::Object::State& s)
{
...
void
AnalogyDriver::doSave(Settings::Object::State& s) const
{
