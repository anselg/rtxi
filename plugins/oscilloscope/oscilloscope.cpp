 ...
 * selection is enabled to change color, style, width and other oscilloscope
 * properties.
 */
...
void
Oscilloscope::Plugin::removeOscilloscopePanel(Panel* d_panel)
{
...
void
Oscilloscope::Panel::receiveEvent(const ::Event::Object* event)
{
  if (event->getName() == Event::IO_BLOCK_INSERT_EVENT) {
    ...
    if (block) {
      ...
      if (index < blocks.size()) {
        ...
        for (std::list<Scope::Channel>::iterator
          ...
          if (info->block == block) {
            ...
            if (trigsChanList->currentText() != "<None>")
              if (i->getLabel() ==
                  scopeWindow->getTriggerChannel()->getLabel()) {
        ...
        DEBUG_MSG(
          "Oscilloscope::Panel::receiveEvent : removed block never inserted\n");
    }
...
void
Oscilloscope::Panel::activateChannel(bool active)
{
...
void
Oscilloscope::Panel::buildChannelList(void)
{
...
void
Oscilloscope::Panel::showTab(int index)
{
  switch (index) {
    case 0:
      showChannelTab();
...
void
Oscilloscope::Panel::applyChannelTab(void)
{
  ...
  if (!activateButton->isChecked()) {
    ...
    scopeWindow->setChannelOffset(i,
...
void
Oscilloscope::Panel::applyDisplayTab(void)
{
...
QWidget*
Oscilloscope::Panel::createChannelTab(QWidget* parent)
{

  setWhatsThis(
    "<p><b>Oscilloscope: Channel Options</b><br>"
    "Use the dropdown boxes to select the signal streams you want to plot from "
    "any loaded modules or your DAQ device. You may change the plotting scale "
    "for "
...
QWidget*
Oscilloscope::Panel::createDisplayTab(QWidget* parent)
{
...
// in the display tab
void
Oscilloscope::Panel::showChannelTab(void)
{
  ...
  for (std::list<Scope::Channel>::iterator i = scopeWindow->getChannelsBegin(),
    ...
    if (info->block && info->block == blocks[blocksList->currentIndex()] &&
      ...
      if (i->getPen().color() == QColor(255, 0, 16, 255))
        ...
        ERROR_MSG(
          "Oscilloscope::Panel::displayChannelTab : invalid color selection\n");
        colorsList->setCurrentIndex(0);
      }
      ...
      switch (i->getPen().width()) {
        ...
        case 5:
          ...
          break;
        default:
          ERROR_MSG("Oscilloscope::Panel::displayChannelTab : invalid width "
                    "selection\n");
          widthsList->setCurrentIndex(0);
      }
      ...
      switch (i->getPen().style()) {
        ...
        case Qt::DashDotDotLine:
          ...
          break;
        default:
          ERROR_MSG("Oscilloscope::Panel::displayChannelTab : invalid style "
                    "selection\n");
          stylesList->setCurrentIndex(0);
      }
      break;
...
void
Oscilloscope::Panel::showDisplayTab(void)
{
  ...
  , fifo(25 * 1048576)
{
  ...
  setWhatsThis(
    ...
    "in your workspace in real-time, including signals from your DAQ card and "
    "those "
    "generated by user modules. Multiple signals are overlaid in the window "
    "and "
    "different line colors and styles can be selected. When a signal is added, "
    "a legend "
    "automatically appears in the bottom of the window. Multiple oscilloscopes "
    "can "
    "be instantiated to give you multiple data windows. To select signals for "
    "plotting, "
...
void
Oscilloscope::Panel::togglePause(void)
{
...
Oscilloscope::Panel::doDeferred(const Settings::Object::State& s)
{
  ...
  for (size_t i = 0, nchans = s.loadInteger("Num Channels"); i < nchans; ++i) {
    ...
    std::list<Scope::Channel>::iterator chan = scopeWindow->insertChannel(
      ...
      QPen(
        QColor(QString::fromStdString(s.loadString(str.str() + " pen color"))),
        s.loadInteger(str.str() + " pen width"),
        Qt::PenStyle(s.loadInteger(str.str() + " pen style"))),
      curve,
      info);

    scopeWindow->setChannelLabel(chan,
                                 info->name + " - " +
                                   scalesList
                                     ->itemText(static_cast<int>(round(
                                       4 * (log10(1 / chan->getScale()) + 1))))
                                     .simplified());
  }
