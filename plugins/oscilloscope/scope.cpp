...
const void*
Scope::Channel::getInfo(void) const
{
  return info;
}
...
// Returns channel pen
QPen
Scope::Channel::getPen(void) const
{
...
double
Scope::Channel::getScale(void) const
{
  return scale;
}
...
double
Scope::Channel::getOffset(void) const
{
  return offset;
}
...
QString
Scope::Channel::getLabel(void) const
{
  return label;
}
...
// settings
std::list<Scope::Channel>::iterator
Scope::insertChannel(QString label,
                     double scale,
                     double offset,
                     const QPen& pen,
                     QwtPlotCurve* curve,
...
void*
Scope::removeChannel(std::list<Scope::Channel>::iterator channel)
{
...
void
Scope::resizeEvent(QResizeEvent* event)
{
...
size_t
Scope::getChannelCount(void) const
{
...
std::list<Scope::Channel>::iterator
Scope::getChannelsEnd(void)
{
  return channels.end();
}

// Returns read-only pointer to beginning of channel list
std::list<Scope::Channel>::const_iterator
Scope::getChannelsBegin(void) const
{
  return channels.begin();
}
...
std::list<Scope::Channel>::const_iterator
Scope::getChannelsEnd(void) const
{
  return channels.end();
}
...
void
Scope::setData(double data[], size_t size)
{
  if (isPaused)
    return;
  ...
  for (std::list<Channel>::iterator i = channels.begin(), end = channels.end();
    ...
    if (triggering && i == triggerChannel &&
        ((triggerDirection == POS && i->data[data_idx - 1] < triggerThreshold &&
         ...
         (triggerDirection == NEG && i->data[data_idx - 1] > triggerThreshold &&
          i->data[data_idx] < triggerThreshold))) {
...
size_t
Scope::getDataSize(void) const
{
  return data_size;
}

void
Scope::setDataSize(size_t size)
{
  for (std::list<Channel>::iterator i = channels.begin(), end = channels.end();
       i != end;
       ++i)
...
Scope::trig_t
Scope::getTriggerDirection(void)
{
  return triggerDirection;
}

double
Scope::getTriggerThreshold(void)
{
  return triggerThreshold;
}

double
Scope::getTriggerWindow(void)
{
  return triggerWindow;
}

std::list<Scope::Channel>::iterator
Scope::getTriggerChannel(void)
{
  return triggerChannel;
}

void
Scope::setTrigger(trig_t direction,
                  double threshold,
...
// Set channel scale
void
Scope::setChannelScale(std::list<Channel>::iterator channel, double scale)
{
  channel->scale = scale;
}
...
// Set channel offset
void
Scope::setChannelOffset(std::list<Channel>::iterator channel, double offset)
{
  channel->offset = offset;
}
...
void
Scope::setChannelPen(std::list<Channel>::iterator channel, const QPen& pen)
{
  channel->curve->setPen(pen);
}
...
// Set channel label
void
Scope::setChannelLabel(std::list<Channel>::iterator channel,
                       const QString& label)
{
