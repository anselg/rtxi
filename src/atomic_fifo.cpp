...
AtomicFifo::write(
  const void* buffer,
  size_t itemSize) // It is an absolute requirement only one thread calls write
{
...
AtomicFifo::read(
  void* buffer,
  size_t itemSize) // It is an absolute requirement only one thread calls read
{
...
size_t
AtomicFifo::increment(size_t current_ptr, size_t itemSize) const
{
  return (current_ptr + itemSize) % fifoSize;
}

bool
AtomicFifo::isLockFree() const
{
