...
size_t
IO::Block::getCount(IO::flags_t type) const
{
  if (type & INPUT)
   return inputs.size();
  if (type & OUTPUT)
...
std::string
IO::Block::getName(IO::flags_t type, size_t n) const
{
...
std::string
IO::Block::getDescription(IO::flags_t type, size_t n) const
{
  if (type & INPUT && n < inputs.size())
...
double
IO::Block::getValue(IO::flags_t type, size_t n) const
{
  if (type & INPUT)
   return input(n);
  if (type & OUTPUT)
...
double
IO::Block::output(size_t n) const
{
...
void
IO::Block::disconnect(IO::Block* src,
                     size_t src_num,
                     IO::Block* dest,
                     size_t dest_num)
{
...
void
IO::Connector::foreachBlock(void (*callback)(Block*, void*), void* param)
{
  Mutex::Locker lock(&mutex);
...
IO::Connector::foreachConnection(
  void (*callback)(Block*, size_t, Block*, size_t, void*),
  void* param)
{
  Mutex::Locker lock(&mutex);
...
void
IO::Connector::connect(IO::Block* src, size_t out, IO::Block* dest, size_t in)
{
  Mutex::Locker lock(&mutex);
...
void
IO::Connector::disconnect(IO::Block* src,
                         size_t out,
                         IO::Block* dest,
                         size_t in)
{
...
bool
IO::Connector::connected(IO::Block* src,
                        size_t src_num,
                        IO::Block* dest,
                        size_t dest_num)
{
...
void
IO::Connector::doDeferred(const Settings::Object::State& s)
{
  Block *src, *dest;
  ...
  for (size_t i = 0, end = s.loadInteger("Num Links"); i < end; ++i) {
   ...
   src = dynamic_cast<Block*>(Settings::Manager::getInstance()->getObject(
     s.loadInteger(str.str() + " Source ID")));
   dest = dynamic_cast<Block*>(Settings::Manager::getInstance()->getObject(
     s.loadInteger(str.str() + " Destination ID")));
   if (src && dest)
     connect(src,
             s.loadInteger(str.str() + " Source channel"),
             dest,
             s.loadInteger(str.str() + " Destination channel"));
  }
}

void
IO::Connector::doSave(Settings::Object::State& s) const
{
  size_t n = 0;

  for (std::list<Block*>::const_iterator i = blockList.begin(),
                                        iend = blockList.end();
      i != iend;
      ++i)
   for (size_t j = 0, jend = (*i)->getCount(INPUT); j < jend; ++j)
     for (std::list<Block::link_t>::const_iterator
            k = (*i)->inputs[j].links.begin(),
            kend = (*i)->inputs[j].links.end();
          k != kend;
          ++k) {
       std::ostringstream str;
       str << n++;
...
void
IO::Connector::insertBlock(IO::Block* block)
{
  if (!block) {
...
void
IO::Connector::removeBlock(IO::Block* block)
{
  if (!block) {
   ERROR_MSG("IO::Connector::insertBlock : invalid block\n");
   return;
