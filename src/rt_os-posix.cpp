...
int
RT::OS::createTask(RT::OS::Task* task, void* (*entry)(void*), void* arg, int)
{
  int retval = 0;
...
void
RT::OS::deleteTask(RT::OS::Task task)
{
...
int
RT::OS::setPeriod(RT::OS::Task task, long long period)
{
  posix_task_t* t = reinterpret_cast<posix_task_t*>(task);
...
void
RT::OS::sleepTimestep(RT::OS::Task task)
{
  posix_task_t* t = reinterpret_cast<posix_task_t*>(task);
  if (t == NULL)
   return;
