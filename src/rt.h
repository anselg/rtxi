...
int
createTask(Task*, void* (*)(void*), void*, int = 0);
void deleteTask(Task);

int
setPeriod(Task, long long);
...
public:
  ...
  iterator begin(void) { return iterator(static_cast<T*>(head)); };
  iterator end(void) { return iterator(static_cast<T*>(&tail)); };
...
template<typename T>
void
List<T>::insert(iterator position, T& node)
{
